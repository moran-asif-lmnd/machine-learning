...
def load_data (test_size):
    diabetes_data = pd.read_csv("pima-indians-diabetes.csv").values
    X = diabetes_data[:, :-1]
    y = diabetes_data[:, -1]
...
def make_predictions(X_test, mean_label_0, std_label_0, mean_label_1, std_label_1):
    probabilities = []
    for x in X_test:
        prob_0 = calculate_probability(x, mean_label_0, std_label_0)
        prob_1 = calculate_probability(x, mean_label_1, std_label_1)
        if prob_0 > prob_1:
...
def main ():
    ...
    y_pred = make_predictions(X_test, mean_label_0, std_label_0, mean_label_1, std_label_1)
